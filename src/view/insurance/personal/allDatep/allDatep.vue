<template>
  <div>
    ...
    <div class="tableList">
      <Table :loading="loading" highlight-row size="large" border stripe :columns="columns" :data="tableLisr" @on-row-dblclick="pdet"></Table>
    </div>
    ...
    if (this.type === 10) {
      axios({
        ...
        for (let i = 0; i < res.data.data.length; i++) {
          ...
          that.tableLisr[i].reminddate = res.data.data[i].reminddate.reminddate.slice(0, indexs)
        }
